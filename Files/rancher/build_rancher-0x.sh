...
# Make sure the proxy allows port 6443, before you proceed
...
case $(uname -n) in
  ...
  ;;
  *)
    sleep $(shuf -i 90-160 -n 1) # allow time for the first node to complete install - make the value random to start the node builds at intervals
...
case $VARIANT in 
  Micro)
    WAIT_TIME=$(shuf -i 10-30 -n 1)
    echo "Pause for: $WAIT_TIME before rebooting"
    sleep $WAIT_TIME
...
## TODO - need to break up this script in to stuff to run on each node vs stuff to run from an external node
## RANCHER FOooo
# Run this from kubernerd
scp 10.10.12.211:.kube/config ~/.kube/local-rancher.kubeconfig
export KUBECONFIG=~/.kube/local-rancher.kubeconfig
sed -i -e "s/127.0.0.1/$MY_K3S_ENDPOINT/g" $KUBECONFIG
